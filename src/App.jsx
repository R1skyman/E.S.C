...
export default function App() {
  ...
  const bootstrap = useCallback(async () => {
    ...
    const auth = await loadJSON("auth", { authed: false });
    const st = await loadJSON("settings", defaultSettings);

    // Load the multi-household array, migrating from the old single-household
    // storage shape if that's all that exists yet (so earlier testing isn't lost).
    let hhList = await loadJSON("households", null);
    if (!hhList) {
      const legacy = await loadJSON("household", null);
      hhList = legacy
        ? [{ id: "h1", name: legacy.children?.[0]?.name ? `${legacy.children[0].name}'s Household` : "My Household", ...legacy }]
        : DEFAULT_HOUSEHOLDS;
  ...
  const persistHouseholds = useCallback(async (nextArr) => {
    setHouseholds(nextArr);
    await saveJSON("households", nextArr);
  }, []);

  // Most existing call sites build a full updated object for the *active* household —
  // this slots it back into the array by matching id, so those call sites don't change.
  const persistHousehold = useCallback(async (next) => {
    const nextArr = households.map((h) => (h.id === next.id ? next : h));
    await persistHouseholds(nextArr);
  }, [households, persistHouseholds]);

  const persistCare = useCallback(async (cid, next) => {
    setCareItems((prev) => ({ ...prev, [cid]: next }));
    await saveJSON(`care:${cid}`, next);
  }, []);

  const persistTimeline = useCallback(async (cid, next) => {
    setTimeline((prev) => ({ ...prev, [cid]: next }));
    await saveJSON(`timeline:${cid}`, next);
  }, []);

  const persistUpcoming = useCallback(async (cid, next) => {
    setUpcoming((prev) => ({ ...prev, [cid]: next }));
    await saveJSON(`upcoming:${cid}`, next);
  }, []);
  ...
  const switchHousehold = useCallback(async (id) => {
    ...
    await saveJSON("activeHouseholdId", id);
    ...
    const newCare = {}, newTl = {}, newUp = {};
    for (const c of hh.children) {
      if (!(c.id in careItems)) newCare[c.id] = await loadJSON(`care:${c.id}`, []);
      if (!(c.id in timeline)) newTl[c.id] = await loadJSON(`timeline:${c.id}`, {});
      if (!(c.id in upcoming)) newUp[c.id] = await loadJSON(`upcoming:${c.id}`, []);
    }
    if (Object.keys(newCare).length) setCareItems((prev) => ({ ...prev, ...newCare }));
    if (Object.keys(newTl).length) setTimeline((prev) => ({ ...prev, ...newTl }));
    if (Object.keys(newUp).length) setUpcoming((prev) => ({ ...prev, ...newUp }));
    // lazy-load this household's Info Bank if we haven't seen it yet
    if (!(id in infoBank)) {
      const ib = await loadJSON(`infoBank:${id}`, []);
      setInfoBank((prev) => ({ ...prev, [id]: ib }));
    }
  }, [households, careItems, timeline, upcoming, infoBank]);

  const addHousehold = useCallback(async (name) => {
    const id = uid();
    const newHH = {
      id, name: name.trim() || "New Household", children: [],
      members: [{ id: uid(), name: "You", initials: "Y", role: "owner", relation: "You", isYou: true }],
      invites: [],
    };
    await persistHouseholds([...households, newHH]);
    await switchHousehold(id);
  }, [households, persistHouseholds, switchHousehold]);
  ...
  const deleteHousehold = useCallback(async (householdIdToDelete) => {
    ...
    // Clean up everything that belonged only to this household
    const keysToDelete = [
      `infoBank:${householdIdToDelete}`,
      ...hh.children.flatMap((c) => [`care:${c.id}`, `timeline:${c.id}`, `upcoming:${c.id}`]),
    ];
    for (const key of keysToDelete) {
      try { await window.storage.delete(key, false); } catch { /* may not exist */ }
    }
    ...
    await persistHouseholds(nextHouseholds);
    if (activeHouseholdId === householdIdToDelete) {
      await switchHousehold(nextHouseholds[0].id);
    }
  }, [households, activeHouseholdId, persistHouseholds, switchHousehold]);
  ...
  const deleteChild = useCallback(async (childIdToDelete) => {
    ...
    await persistHousehold(nextHousehold);
    for (const key of [`care:${childIdToDelete}`, `timeline:${childIdToDelete}`, `upcoming:${childIdToDelete}`]) {
      try { await window.storage.delete(key, false); } catch { /* may not exist */ }
    }
  ...
  const handleLoginSuccess = async (remember = true) => {
    setAuthed(true);
    setUnlocked(true); // just proved identity via password/face/passcode — no need to gate again immediately
    if (remember) {
      await saveJSON("auth", { authed: true });
    }
    // If not remembered, stay logged in for this session only — nothing is written to
    // storage, so closing and reopening the app will require logging in again.
  };
  ...
  const persistInfoBank = useCallback(async (next) => {
    ...
    await saveJSON(`infoBank:${activeHouseholdId}`, next);
  }, [activeHouseholdId]);
  ...
  const logout = useCallback(async () => {
    ...
    setUnlocked(false);
    setTab("today");
    await saveJSON("auth", { authed: false });
  }, []);

  const deleteAllData = useCallback(async () => {
    const allChildren = households.flatMap((h) => h.children);
    const keys = [
      "auth", "household", "households", "activeHouseholdId", "settings", "rememberMePref", "infoBank",
      ...allChildren.flatMap((c) => [`care:${c.id}`, `timeline:${c.id}`, `upcoming:${c.id}`]),
      ...households.map((h) => `infoBank:${h.id}`),
    ];
    for (const k of keys) {
      try { await window.storage.delete(k, false); } catch { /* key may not exist */ }
    }
    setAuthed(false);
    ...
    <div className="flex justify-center" style={{ fontFamily: "-apple-system, sans-serif", minHeight: viewportH, backgroundColor: "#F7F9FB" }}>
      <div className="w-full max-w-[420px] relative overflow-hidden flex flex-col" style={{ height: viewportH, backgroundColor: "#F7F9FB" }}>
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden pb-24">
          ...
          {tab === "settings" && (
            <SettingsScreen settings={settings} persistSettings={persistSettings} onLogout={logout} onDeleteAllData={deleteAllData} onSimulateReopen={bootstrap}
...
function LoginScreen({ viewportH, onSuccess, faceIdAuto }) {
  ...
  // This preference owns its own dedicated storage key and loads/saves itself directly.
  useEffect(() => {
    (async () => {
      const stored = await loadJSON("rememberMePref", { remember: true });
      setRememberMe(stored.remember !== false);
    })();
  }, []);
  ...
  const toggleRemember = () => {
    ...
    setRememberMe(next);
    saveJSON("rememberMePref", { remember: next });
  };
  ...
  const startScan = () => {
    ...
    scanTimer.current = setInterval(() => {
      ...
      if (p >= 100) { clearInterval(scanTimer.current); setTimeout(() => { setStep("success"); setTimeout(() => onSuccess(rememberMe), 700); }, 300); }
    }, 40);
  };
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 flex flex-col justify-center">
          {step === "login" && (
            <div>
              ...
              <form onSubmit={(e) => { e.preventDefault(); if (canSubmit) onSuccess(rememberMe); }} className="flex flex-col gap-3">
                ...
                <button type="button" onClick={toggleRemember} className="flex items-center gap-2 px-0.5" style={{ backgroundColor: "transparent", WebkitAppearance: "none" }}>
                ...
                <div className="mt-1 flex" style={{ borderRadius: 16, overflow: "hidden" }}>
                  <button type="submit" disabled={!canSubmit} className="flex-1 py-3.5 flex items-center justify-center gap-2 font-semibold text-[15px]"
                    style={{ WebkitAppearance: "none", backgroundColor: canSubmit ? "#4A7FAE" : "#C4CDD6", color: "white" }}>
                    Log in <ArrowRight size={17} />
                  </button>
                  <button type="button" onClick={() => setStep("faceid")} className="w-14 flex items-center justify-center shrink-0"
              ...
              <button type="button" onClick={() => setStep("signup")} className="block mx-auto text-[13px] text-[#8A94A0] mt-3" style={{ backgroundColor: "transparent", WebkitAppearance: "none" }}>
            ...
            <div>
              ...
              {!forgotSent ? (
                <>
                  ...
                  <button
                    onClick={() => forgotEmail.trim().length > 3 && setForgotSent(true)}
          ...
          {step === "signup" && (
            <div>
              ...
              <div className="flex flex-col gap-3">
                ...
                <button
                  onClick={() => onSuccess(rememberMe)}
                  disabled={!(signupName.trim() && signupEmail.trim().length > 3 && signupPassword.length >= 6 && agreedToTerms)}
                  className="mt-1 rounded-2xl py-3.5 font-semibold text-[15px]"
                  style={{
                    WebkitAppearance: "none",
                    backgroundColor: (signupName.trim() && signupEmail.trim().length > 3 && signupPassword.length >= 6 && agreedToTerms) ? "#4A7FAE" : "#C4CDD6",
                    color: "white",
                  }}
                >
                  Create household
          ...
          {step === "faceid" && (
            <div className="text-center">
              ...
              <button onClick={() => { setStep("success"); setTimeout(() => onSuccess(rememberMe), 700); }} className="block mx-auto mt-6 text-[13px] font-semibold text-[#8A94A0] flex items-center gap-1"
...
function HouseholdScreen({ household, persistHousehold, addChild, editChild, deleteChild, myRole, households, activeHouseholdId, switchHousehold, addHousehold, deleteHousehold }) {
  ...
  const acceptInvite = async (inviteId) => {
    ...
    const next = {
      ...household,
      members: [...household.members, { id: uid(), name, initials: name[0]?.toUpperCase() || "?", role: inv.role || "view", relation: inv.relation }],
